...
let _draftSort             = 'manual';
let _draftStatusFilter     = 'all';
let _draftImages           = [];   // array of image URLs (base64 data URLs or /api/draft-images/ server URLs)
let _draftSelectedIdx      = 0;    // index of the active/main image
...
function setDraftMode(mode) {
  ...
  if (mode === 'view') {
    ...
    actions.innerHTML = `
      ...
      <button class="ai-only btn btn-ghost" id="draft-ai-btn" onclick="withBtn(this, askGeminiForDraft)"
        style="color:#8b5cf6;border-color:rgba(139,92,246,.4)">
        <i class="fas fa-wand-magic-sparkles"></i> ${t('draft.ai_suggest')}
      </button>
      <label class="ai-only" title="${t('draft.ai_use_stock_hint')}" style="display:flex;align-items:center;gap:5px;font-size:.82rem;cursor:pointer;user-select:none;color:var(--muted)">
...
function renderDraftView() {
  ...
  if (d.target_date) {
    ...
    bannerHtml = `<div style="display:flex;flex-direction:column;gap:5px;margin-bottom:16px;
      ...
      <div style="display:flex;align-items:center;gap:8px">
        <i class="fas fa-calendar-day" style="color:#8b5cf6;width:14px;text-align:center"></i>
  ...
  // Galerie images (vue lecture)
  const imgHtml = _draftImages.length > 0
    ? `<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px">
        ${_draftImages.map((src, idx) => {
          const sel = idx === _draftSelectedIdx;
          return `<div style="position:relative;border-radius:10px;overflow:hidden;
            border:2px solid ${sel ? 'var(--accent)' : 'transparent'};flex-shrink:0">
            <img src="${src}" style="width:100px;height:100px;object-fit:cover;display:block;
              cursor:zoom-in" onclick="openDraftLightbox(${idx})"
              title="${esc(t('common.zoom_in'))}">
            ${sel ? `<span style="position:absolute;bottom:3px;right:3px;background:var(--accent);color:#fff;
              border-radius:50%;width:16px;height:16px;display:flex;align-items:center;justify-content:center;
              font-size:.5rem;pointer-events:none"><i class="fas fa-check"></i></span>` : ''}
          </div>`;
        }).join('')}
       </div>`
    : '';
  ...
  panel.innerHTML = `
    ...
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px">
      ...
      <h3 style="margin:0;font-size:1.25rem;font-weight:700">${esc(d.title || t('draft.untitled'))}</h3>
    </div>
    <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:16px">
      ...
      ${(() => { const s = d.status || 'idea'; const c = _DRAFT_STATUS_COLOR[s]; return `<span class="badge" style="background:color-mix(in srgb,${c} 15%,transparent);color:${c};border:1px solid color-mix(in srgb,${c} 30%,transparent)"><i class="fas ${_DRAFT_STATUS_ICON[s]}" style="font-size:.65rem"></i> ${esc(_draftStatusLabel(s))}</span>`; })()}
    </div>
    ...
    <div style="margin-bottom:16px">
      <div style="font-size:.78rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);margin-bottom:8px">${t('draft.ingredients')}</div>
      ${ingHtml}
    </div>` : ''}

    ${d.notes ? `
    <div style="margin-bottom:8px">
      <div style="font-size:.78rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);margin-bottom:6px">${t('draft.field_notes')}</div>
...
// ── Statut helpers ────────────────────────────────────────────────────────────
const _DRAFT_STATUSES = ['idea', 'in_progress', 'ready'];
const _DRAFT_STATUS_COLOR = { idea: 'var(--muted)', in_progress: 'var(--amber)', ready: '#10b981' };
const _DRAFT_STATUS_ICON  = { idea: 'fa-lightbulb', in_progress: 'fa-pen-ruler', ready: 'fa-check-circle' };

function _draftStatusLabel(s) {
  if (s === 'in_progress') return t('draft.status_in_progress');
  if (s === 'ready')       return t('draft.status_ready');
  return t('draft.status_idea');
}

function _renderDraftStatusFilter() {
  const bar = document.getElementById('draft-status-filter');
  if (!bar) return;
  const all = ['all', ..._DRAFT_STATUSES];
  bar.innerHTML = all.map(s => {
    const active = s === _draftStatusFilter;
    const label  = s === 'all' ? t('draft.filter_all') : _draftStatusLabel(s);
    const color  = s === 'all' ? 'var(--text)' : _DRAFT_STATUS_COLOR[s];
    return `<button onclick="_draftStatusFilter='${s}';_renderDraftStatusFilter();renderBrouillons()"
      style="font-size:.72rem;padding:3px 9px;border-radius:20px;border:1px solid ${active ? color : 'var(--border)'};
        background:${active ? `color-mix(in srgb,${color} 15%,transparent)` : 'transparent'};
        color:${active ? color : 'var(--muted)'};cursor:pointer;white-space:nowrap;transition:.15s">
      ${s !== 'all' ? `<i class="fas ${_DRAFT_STATUS_ICON[s]}" style="font-size:.65rem"></i> ` : ''}${esc(label)}
    </button>`;
  }).join('');
}
...
function renderBrouillons() {
  ...
  _renderDraftStatusFilter();
  const q = (document.getElementById('draft-search')?.value || '').toLowerCase();
  const total = S.drafts.length;
  let shown = S.drafts.filter(d => {
    if (_draftStatusFilter !== 'all' && (d.status || 'idea') !== _draftStatusFilter) return false;
    if (q && !(d.title || '').toLowerCase().includes(q) && !(d.style || '').toLowerCase().includes(q)) return false;
    return true;
  });

  _draftSort = document.getElementById('draft-sort')?.value || _draftSort;
  const isManual = _draftSort === 'manual';
  if (_draftSort === 'recent') {
    shown.sort((a, b) => (b.updated_at || '').localeCompare(a.updated_at || ''));
  } else if (_draftSort === 'alpha') {
    shown.sort((a, b) => (a.title || '').localeCompare(b.title || '', _lang || 'fr'));
  } else if (_draftSort === 'target') {
    shown.sort((a, b) => {
  ...
  if (!shown.length) {
    ...
    list.innerHTML = shown.map(d => {
      ...
      const dStatus = d.status || 'idea';
      const statusBadge = dStatus !== 'idea'
        ? `<span style="font-size:.65rem;padding:1px 6px;border-radius:10px;flex-shrink:0;
             background:color-mix(in srgb,${_DRAFT_STATUS_COLOR[dStatus]} 15%,transparent);
             color:${_DRAFT_STATUS_COLOR[dStatus]};border:1px solid color-mix(in srgb,${_DRAFT_STATUS_COLOR[dStatus]} 35%,transparent)">
            <i class="fas ${_DRAFT_STATUS_ICON[dStatus]}" style="font-size:.6rem"></i> ${esc(_draftStatusLabel(dStatus))}
           </span>` : '';
      let ingCount = 0;
      ...
      if (d.target_date) {
        ...
        dateLine = `<div style="display:flex;flex-direction:column;gap:3px;margin-top:5px;padding-left:26px">
          ...
          <div style="display:flex;align-items:center;gap:5px">
            <i class="fas fa-calendar-day" style="color:#8b5cf6;font-size:.62rem;flex-shrink:0;width:10px;text-align:center"></i>
      ...
      return `<div class="card draft-list-item" draggable="true" data-id="${d.id}"
        ...
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:2px">
          ${isManual ? `<span class="draft-drag-handle" onclick="event.stopPropagation()" title="${t('rec.drag_to_reorder')}"
...
async function openDraft(id) {
  // Ne pas laisser un debounce de sauvegarde du brouillon précédent s'exécuter
  // après le changement de contexte : il écrirait le contenu du nouveau
  // formulaire sur l'ancien _currentDraftId (déjà réassigné) une fois le
  // timer déclenché — flush immédiat à la place (même logique que draftToRecipe()).
  if (_currentDraftId && _currentDraftId !== id) {
  ...
  document.getElementById('draft-status').value = d.status || 'idea';
  document.getElementById('draft-save-status').textContent = '';
  _renderDraftEventLink(d.target_date || null, d.event_label || null);
  // Galerie images — images incluses dans la réponse de liste depuis get_drafts
  _draftImages = [];
  _draftSelectedIdx = 0;
  try { _draftImages = JSON.parse(d.images || '[]'); } catch(e) { _draftImages = []; }
  if (!Array.isArray(_draftImages)) _draftImages = [];
  if (d.image && _draftImages.length) {
    const si = _draftImages.indexOf(d.image);
    _draftSelectedIdx = si >= 0 ? si : 0;
  }
  renderDraftImageGallery();
...
async function newDraft() {
  // Même flush que openDraft() : ne pas laisser le debounce du brouillon
  // précédent écrire sur le nouveau une fois qu'il se déclenchera.
  if (_currentDraftId) {
    clearTimeout(_draftSaveTimer);
    await saveDraft();
  }
...
async function saveDraft() {
  ...
  const payload = {
    ...
    images:      JSON.stringify(_draftImages),
    status:      document.getElementById('draft-status').value || 'idea',
  };
  try {
    ...
    // Sync _draftImages with server URLs (replaces any base64 blobs with /api/draft-images/ URLs)
    try {
      const serverImgs = JSON.parse(updated.images || '[]');
      if (Array.isArray(serverImgs)) {
        _draftImages = serverImgs;
        renderDraftImageGallery();
      }
    } catch(_) {}
    const el = document.getElementById('draft-save-status');
...
// ── Lightbox images brouillon ────────────────────────────────────────────────
let _draftLbIdx = 0;

function openDraftLightbox(idx) {
  _draftLbIdx = idx;
  _updateDraftLightbox();
  const lb = document.getElementById('draft-lightbox');
  if (!lb) return;
  lb.style.display = 'flex';
  document.body.style.overflow = 'hidden';
}

function closeDraftLightbox() {
  const lb = document.getElementById('draft-lightbox');
  if (lb) lb.style.display = 'none';
  document.body.style.overflow = '';
}

function draftLbNav(dir) {
  if (!_draftImages.length) return;
  _draftLbIdx = (_draftLbIdx + dir + _draftImages.length) % _draftImages.length;
  _updateDraftLightbox();
}

function _updateDraftLightbox() {
  const img     = document.getElementById('draft-lb-img');
  const counter = document.getElementById('draft-lb-counter');
  const prev    = document.getElementById('draft-lb-prev');
  const next    = document.getElementById('draft-lb-next');
  if (!img) return;
  img.src = _draftImages[_draftLbIdx] || '';
  const multi = _draftImages.length > 1;
  if (counter) counter.textContent = multi ? `${_draftLbIdx + 1} / ${_draftImages.length}` : '';
...
document.addEventListener('keydown', e => {
  ...
  if (e.key === 'Escape')      { e.stopPropagation(); closeDraftLightbox(); }
  else if (e.key === 'ArrowLeft')  draftLbNav(-1);
  else if (e.key === 'ArrowRight') draftLbNav(1);
}, true);

// ── Galerie d'images du brouillon ─────────────────────────────────────────────
function renderDraftImageGallery() {
  const gallery = document.getElementById('draft-img-gallery');
  if (!gallery) return;
  const S = 88; // tile size px
  const tiles = _draftImages.map((src, idx) => {
    const sel = idx === _draftSelectedIdx;
    return `<div style="position:relative;width:${S}px;height:${S}px;flex-shrink:0;border-radius:8px;
              overflow:hidden;border:2px solid ${sel ? 'var(--accent)' : 'var(--border)'};
              cursor:pointer;transition:border-color .15s"
              onclick="selectDraftImage(${idx})" title="${esc(t('draft.img_select'))}">
      ...
      ${sel ? `<span style="position:absolute;bottom:3px;right:3px;background:var(--accent);color:#fff;
        ...
        font-size:.55rem;pointer-events:none"><i class="fas fa-check"></i></span>` : ''}
      <button onclick="event.stopPropagation();openDraftLightbox(${idx})"
        title="${esc(t('common.zoom_in') || '🔍')}"
        style="position:absolute;top:3px;left:3px;background:rgba(0,0,0,.6);border:none;border-radius:50%;
          width:20px;height:20px;display:flex;align-items:center;justify-content:center;
          cursor:zoom-in;padding:0;color:#fff;font-size:.6rem">
        <i class="fas fa-magnifying-glass-plus"></i>
      </button>
      <button onclick="event.stopPropagation();removeDraftImageAt(${idx})"
        title="${esc(t('draft.img_remove'))}"
        style="position:absolute;top:3px;right:3px;background:rgba(0,0,0,.6);border:none;border-radius:50%;
          width:20px;height:20px;display:flex;align-items:center;justify-content:center;
          cursor:pointer;padding:0;color:#fff;font-size:.6rem">
        <i class="fas fa-times"></i>
      </button>
    </div>`;
  });
  // Tile "ajouter"
  tiles.push(`<div onclick="document.getElementById('draft-img-input').click()"
    title="${esc(t('draft.img_click_add'))}"
    style="width:${S}px;height:${S}px;flex-shrink:0;border-radius:8px;border:2px dashed var(--border);
      display:flex;flex-direction:column;align-items:center;justify-content:center;gap:5px;
      color:var(--muted);cursor:pointer;transition:border-color .15s"
    onmouseenter="this.style.borderColor='var(--accent)'"
    onmouseleave="this.style.borderColor='var(--border)'">
    <i class="fas fa-plus" style="font-size:1.1rem;opacity:.4"></i>
    <span style="font-size:.65rem;opacity:.5">${esc(t('draft.img_click_add'))}</span>
  </div>`);
  gallery.innerHTML = tiles.join('');
}

function selectDraftImage(idx) {
  if (idx === _draftSelectedIdx) return;
  _draftSelectedIdx = idx;
  renderDraftImageGallery();
  if (_draftMode === 'view') renderDraftView();
  scheduleDraftSave();
}

function addDraftImage(dataUrl) {
  if (!dataUrl) return;
  _draftImages.push(dataUrl);
  _draftSelectedIdx = _draftImages.length - 1;
  renderDraftImageGallery();
  if (_draftMode === 'view') renderDraftView();
  scheduleDraftSave();
}

function removeDraftImageAt(idx) {
  _draftImages.splice(idx, 1);
  // Une image supprimée avant la sélection décale tout ce qui suit : sans ce
  // recalage, _draftSelectedIdx continue de pointer sur l'index numérique
  // d'origine, qui correspond maintenant à une autre photo.
  if (idx < _draftSelectedIdx) {
    _draftSelectedIdx--;
  }
  if (_draftSelectedIdx >= _draftImages.length) {
    _draftSelectedIdx = Math.max(0, _draftImages.length - 1);
  }
  renderDraftImageGallery();
  if (_draftMode === 'view') renderDraftView();
  scheduleDraftSave();
...
function loadDraftImageFile(input) {
  ...
  reader.onload = e => addDraftImage(e.target.result);
...
async function generateDraftImage() {
  ...
  try {
    ...
    addDraftImage(`data:${mime};base64,${b64}`);
  ...
  try {
    ...
    if (provider === 'openai') {
      ...
      if (!result.candidates?.length) {
        throw new Error(result.promptFeedback?.blockReason || t('draft.ai_empty_response'));
      }
    ...
    if (recipe.notes) {
      ...
      notesEl.value  = existing ? `${existing}\n\n${t('draft.ai_notes_sep').replace('${prov}', provLabel)}\n${recipe.notes}` : recipe.notes;
    }
