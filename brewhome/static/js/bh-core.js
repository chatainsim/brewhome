...
// Échappement pour une chaîne JS placée entre apostrophes DANS un attribut
// HTML — typiquement onclick="f('...')". esc() seul n'y suffit pas : il ne
// touche pas à l'apostrophe, qui referme la chaîne JS. L'ordre compte, les
// antislashs d'abord, sinon on échapperait les antislashs qu'on vient d'ajouter.
...
function renderSettingsCatalog(cat) {
  ...
  const searchEl = document.getElementById('settings-catalog-search');
...
function filterSettingsCatalog() {
  const q    = (document.getElementById('settings-catalog-search')?.value || '').toLowerCase();
  const cat  = settingsCat;
  const all  = S.catalog.filter(c => c.category === cat)
    .sort((a,b) => (a.subcategory||'').localeCompare(b.subcategory||'') || a.name.localeCompare(b.name));
  const items = q ? all.filter(c => c.name.toLowerCase().includes(q) || (c.subcategory||'').toLowerCase().includes(q)) : all;
  const list  = document.getElementById('settings-catalog-list');
  if (!list) return;
  list.innerHTML = items.length
    ? items.map(item => _catalogRowHtml(item, cat)).join('')
    : `<div style="color:var(--muted);text-align:center;padding:20px 0;font-size:.87rem">${q ? t('common.no_results') : t('inv.empty')}</div>`;
}
...
function renderSettingsThresholds() {
  ...
  const bsz = bottleSizesEnabled();
  BOTTLE_SIZES.forEach(size => {
    const el = document.getElementById(`bsz-${size}`);
    if (el) el.checked = !!bsz[size];
  });
}
...
function saveThresholds() {
  ...
  appSettings.bottleSizes = {};
  BOTTLE_SIZES.forEach(size => {
    appSettings.bottleSizes[size] = !!document.getElementById(`bsz-${size}`)?.checked;
  });
  saveSettings();
  renderInventaire();
  renderCave();
...
const WATER_FIELDS = ['price','cooling','ph','ca','mg','na','so4','cl','hco3'];
    ...
    if (!Object.keys(fetched).length) {
      ...
      resultDiv.innerHTML = `
        ...
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          ...
          <button class="btn btn-ghost btn-sm" onclick="showHubEauSaveForm('${escJsAttr(communeText)}')">
...
(async () => {
  ...
  document.querySelectorAll('.nav-group').forEach(group => {
    ...
    mainBtn.addEventListener('touchstart', function(e) {
      if (!group.classList.contains('dd-open')) {
        ...
        // Barre de navigation en bas (mobile) : le menu s'ouvre au-dessus
        if (rect.top > window.innerHeight / 2) {
          const barTop = (mainBtn.closest('.nav-links') || mainBtn).getBoundingClientRect().top;
          dropdown.style.top    = 'auto';
          dropdown.style.bottom = (window.innerHeight - barTop + 6) + 'px';
        } else {
          dropdown.style.top    = rect.bottom + 'px';
        }
        dropdown.style.left     = Math.max(6, Math.min(rect.left, window.innerWidth - 190)) + 'px';
  ...
  // /#settings-<onglet> est un cas particulier : Réglages est une modale, pas une
  // page-*, donc pas éligible à navigate() - ouvre la modale puis bascule sur l'onglet
  // demandé (utilisé par le bouton "Synchroniser la cave" de l'app Android, qui a
  // besoin d'atterrir directement sur l'onglet GitHub plutôt que sur Catalogue).
  const hashPage = location.hash.replace(/^#\/?/, '');
  if (hashPage.startsWith('settings-')) {
    openSettings().then(() => settingsTab(hashPage.slice('settings-'.length)));
  ...
  window.addEventListener('hashchange', () => {
    ...
    if (p.startsWith('settings-')) {
      openSettings().then(() => settingsTab(p.slice('settings-'.length)));
      return;
    }
