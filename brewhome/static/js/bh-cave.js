...
function renderCave() {
  ...
  const bottleTotals = {};
  BOTTLE_SIZES.forEach(size => { bottleTotals[size] = active.reduce((s,b) => s + (b[`stock_${size}`]||0), 0); });
  const totalKeg = active.reduce((s,b) => s + (b.keg_liters||0), 0);
  const totalL  = active.reduce((s,b) => s + beerLiters(b), 0).toFixed(1);
  ...
  const sizeStatColors = { '25cl': '#c084fc', '33cl': 'var(--info)', '50cl': '#f472b6', '75cl': 'var(--amber)' };
  const sizeStatsHtml = BOTTLE_SIZES
    .filter(size => isSizeEnabled(size) || bottleTotals[size] > 0)
    .map(size => `<div class="stat"><div class="stat-val" style="color:${sizeStatColors[size]}">${bottleTotals[size]}</div><div class="stat-lbl">${t(`cave.stat_bottles_${size.replace('cl','')}`)}</div></div>`)
    .join('');
  ...
  const beerCardHtml = b => {
        ...
        if (totalCost !== null) {
          ...
          if (costPerL !== null && volume > 0) {
            ...
            const stockVHtml = stockV !== null && stockV > 0
              ...
              : '';
            const pricePerSizeHtml = BOTTLE_SIZES
              .filter(size => isSizeUsable(size, b))
              .map(size => `<span style="color:var(--border)">·</span><span style="color:var(--muted)">${size} <strong style="color:var(--success)">${(costPerL * BOTTLE_SIZE_LITERS[size]).toFixed(2)}€</strong></span>`)
              .join('');
            priceHtml = `
    ...
    return `
      <div class="beer-card ${b.archived ? 'archived-item' : (beerLiters(b)===0?'beer-card--empty':'')}" data-id="${b.id}" draggable="true" style="position:relative">
        ...
        <div class="beer-body">
          ...
          <div class="beer-stocks">
            ${BOTTLE_SIZES.filter(size => isSizeUsable(size, b)).map(size => beerStockHtml(b[`stock_${size}`], b[`initial_${size}`], size)).join('')}
          </div>
          ${_deplBadge(b.id)}
          ${priceHtml}
          ...
          <div style="display:flex;gap:5px;margin-bottom:5px">
            ${BOTTLE_SIZES.filter(size => isSizeUsable(size, b)).map(size =>
              `<button class="btn btn-sm btn-ghost" onclick="withBtn(this,()=>adjustStock(${b.id},'${size}',-1))" title="−1 bouteille ${size}" style="flex:1"><i class="fas fa-minus"></i> ${size}</button>`
            ).join('')}
          </div>
  ...
  const hasStock  = shown.filter(b => beerLiters(b) > 0);
  const noStock   = shown.filter(b => beerLiters(b) === 0);
  ...
  try {
    ...
    if (stocksEl) {
      const kegBlock = (b.keg_liters || b.keg_initial_liters)
        ...
        : '';
      const sizeDetailColors = { '25cl': '#c084fc', '33cl': 'var(--info)', '50cl': '#f472b6', '75cl': 'var(--amber)' };
      const sizeBlocks = BOTTLE_SIZES.filter(size => isSizeUsable(size, b)).map(size => `
        <div style="background:var(--card2);border-radius:12px;padding:14px;text-align:center">
          <div style="font-size:2rem;font-weight:800;color:${sizeDetailColors[size]}">${b[`stock_${size}`] || 0}</div>
          <div style="font-size:.8rem;color:var(--muted);margin-top:2px">${t(`cave.bottles_label_${size.replace('cl','')}`)}</div>
          ${b[`initial_${size}`] ? `<div style="font-size:.72rem;color:var(--muted);margin-top:4px">/ ${b[`initial_${size}`]} initial</div>` : ''}
        </div>`).join('');
      stocksEl.innerHTML = kegBlock + sizeBlocks;
    }
    ...
    // Meta
    const totalL = beerLiters(b).toFixed(2);
    const metaParts = [`<i class="fas fa-wine-bottle"></i> ${totalL} L en cave`];
    const fmtDate = d => d ? d.split('-').reverse().join('/') : null;
    if (b.brew_date)        metaParts.push(`<i class="fas fa-fire-flame-curved"></i> Brassée le ${fmtDate(b.brew_date)}`);
    if (b.bottling_date)    metaParts.push(`<i class="fas fa-wine-bottle"></i> Embouteillée le ${fmtDate(b.bottling_date)}`);
...
function printBeerLabel(id) {
  _labelBeerId = id;
  const b = S.beers.find(x => x.id === id);
  const totalBottles = b ? BOTTLE_SIZES.reduce((s, size) => s + (b[`stock_${size}`] || 0), 0) : 0;
...
function _validateBeerFields() {
  const checks = [
    ...
    ...BOTTLE_SIZES.map(size => ({ id: `beer-f-${size}`, label: size, min: 0, max: 9999, optional: false })),
...
async function adjustStock(id, size, delta) {
  const beer = S.beers.find(b => b.id === id);
  if (!beer) return;
  const payload = { [`stock_${size}`]: Math.max(0, (beer[`stock_${size}`]||0) + delta) };
  try {
...
function updateKegCalc() {
  ...
  const bottledDetail = BOTTLE_SIZES.filter(size => bottles[size]).map(size => `${bottles[size]}×${size}`).join(' + ') || '—';
  document.getElementById('keg-tr-calc').innerHTML = `
    <div style="display:grid;grid-template-columns:1fr auto;gap:2px 12px">
      ...
      <span style="color:var(--muted)">Embouteillé (${bottledDetail})</span><span><strong>${bottled.toFixed(2)} L</strong></span>
...
async function saveKegTransfer() {
  ...
  const payload = { keg_liters: Math.max(0, parseFloat((available - total).toFixed(3))) };
  BOTTLE_SIZES.forEach(size => { payload[`stock_${size}`] = (beer[`stock_${size}`] || 0) + bottles[size]; });
  try {
