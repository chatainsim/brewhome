...
function renderDashWidgets() {
  ...
  if (!upcoming.length) {
    wEvent = `<div class="card" style="height:100%">
      <div class="section-label" style="margin-bottom:10px"><i class="fas fa-calendar-day" style="color:var(--info)"></i> ${t('dash.wgt_event')}</div>
    ...
    wEvent = `<div class="card" style="height:100%">
      <div class="section-label" style="margin-bottom:8px"><i class="fas fa-calendar-day" style="color:var(--info)"></i> ${t('dash.wgt_event')}</div>
      ${rows}
    </div>`;
  }
...
function _invRowHtml(item) {
  ...
  return `<tr class="inv-row ${item.archived ? 'archived-item' : ''}${isEmpty ? ' inv-row-empty' : ''}" draggable="true" data-id="${item.id}">
        <td class="inv-drag-col" style="padding:2px 4px;width:18px"><span class="inv-drag-handle" onmousedown="_invDragFromHandle=true" onmouseup="_invDragFromHandle=false"><i class="fas fa-grip-vertical"></i></span></td>
...
let _invAC = null;
let _invSort = { field: null, dir: 1 }; // field: 'name'|'qty'|null, dir: 1=asc -1=desc

function setInvSort(field) {
  if (_invSort.field === field) {
    if (_invSort.dir === 1) { _invSort.dir = -1; }
    else { _invSort.field = null; _invSort.dir = 1; }
  } else {
  ...
  ['name','qty'].forEach(f => {
    const th = document.getElementById('inv-th-' + f);
    if (!th) return;
    const icon = th.querySelector('.sort-icon');
    if (_invSort.field === f) {
      icon.className = 'sort-icon fas fa-arrow-' + (_invSort.dir === 1 ? 'up' : 'down');
...
function renderInventaire(searchOnly = false) {
  ...
  if (_invSort.field) {
    shown = [...shown].sort((a, b) => {
      if (_invSort.field === 'name') return _invSort.dir * a.name.localeCompare(b.name);
      if (_invSort.field === 'qty')  return _invSort.dir * ((a.quantity || 0) - (b.quantity || 0));
      return 0;
    });
  }

  // Afficher/masquer la colonne drag handle selon le tri
  const dragCols = document.querySelectorAll('.inv-drag-col');
  dragCols.forEach(el => el.style.display = _invSort.field ? 'none' : '');
  ...
  // Drag-and-drop reorder (désactivé quand un tri est actif)
  if (_invAC) _invAC.abort();
  if (_invSort.field) return;
