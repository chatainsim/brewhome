  ...
  if (alertEl) {
    ...
    if (items.length) {
      ...
      alertEl.innerHTML = `<div style="padding:8px 14px;background:${bg};border:1px solid ${border};border-radius:10px;font-size:.82rem;display:flex;align-items:center;gap:10px;flex-wrap:wrap">
        ...
        <span style="color:var(--muted);white-space:nowrap">${t('keg.revision_alert_title')}
          <span style="display:inline-flex;align-items:center;justify-content:center;background:${color};color:#fff;border-radius:20px;padding:1px 7px;font-weight:700;font-size:.72rem;margin-left:4px">${items.length}</span> :
        </span>
        ${parts}
      </div>`;
    } else {
...
async function openBrewFermentationChart(brewId) {
  ...
  try {
    ...
    const isDone = brew?.status === 'completed';

    // OG = max des mesures de la fenêtre affichée (delta ABV cohérent avec le graphique visible)
    const _validGravities = gravities.filter(g => g != null);
    const og = _validGravities.length ? Math.max(..._validGravities) : null;
    const abvChartLabel = t('brew.abv_chart_label').replace('${og}', og != null ? og.toFixed(3) : '—');
    const abvData = gravities.map(g => (g != null && og != null) ? parseFloat(((og - g) * 131.25).toFixed(2)) : null);
      ...
      options: {
        ...
        plugins: {
          legend: { labels: { color: '#ccc', font: { size: 12 },
            filter: item => item.datasetIndex < (isDone ? 2 : 3 + (targetTempData ? 1 : 0)) } },
          tooltip: {
            callbacks: {
              label: ctx => {
        ...
        scales: {
          ...
          yTemp: { type: 'linear', position: 'right',
            display: !isDone && (temps.some(v => v != null) || !!targetTempData),
...
function openBrewCostModal(brewId) {
  ...
  const perSizeRows = BOTTLE_SIZES.map(size => cd.perSize?.[size] != null
    ? `<div style="display:flex;align-items:center;gap:6px;padding:4px 0">
        <i class="fas fa-wine-bottle" style="color:var(--muted);font-size:.75rem;width:14px"></i>
        <span style="flex:1;font-size:.8rem;color:var(--muted)">${t(`brew.cost_per_bottle_${size.replace('cl','')}`)}</span>
        <strong style="font-size:.95rem;color:var(--success)">${cd.perSize[size].toFixed(2)} €</strong>
      </div>` : '').join('');
  ...
  body.innerHTML = `
    ...
    <div style="display:flex;flex-direction:column;gap:2px;padding:6px 10px;background:var(--card2);border-radius:8px">
      ${perLiterRow}${perSizeRows}
      ${!perLiterRow && !perSizeRows ? `<div style="font-size:.78rem;color:var(--muted);text-align:center;padding:4px 0">${t('brew.cost_no_volume')}</div>` : ''}
    </div>`;
