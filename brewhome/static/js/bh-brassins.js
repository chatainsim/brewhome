...
function _renderLifecycle(b) {
  ...
  const totalStock = linkedBeers.reduce((s, bx) => s + beerLiters(bx), 0);
  const stockHtml = linkedBeers.length ? `<div style="margin-top:16px;padding:10px 14px;border-radius:8px;background:var(--card2);border:1px solid var(--border);font-size:.82rem;color:var(--muted)">
    ...
    ${linkedBeers.map(bx => {
      const parts = BOTTLE_SIZES.filter(size => bx[`stock_${size}`]).map(size => `${bx[`stock_${size}`]}×${size}`);
...
async function markDryhopDone(brewId, dateStr) {
  try {
    ...
    // Le stock des houblons de dry hop est déduit côté serveur : rafraîchir l'inventaire local
    if (res.deducted?.length) {
      try { S.inventory = await api('GET', '/api/inventory'); } catch(e) {}
    }
    renderBrassins();
...
// ── Calcul optimal bouteilles (tailles activées) ─────────────────────────────
// Retourne {<size>: n, ..., wasteCl} pour minimiser le volume non embouteillé.
// Contrainte : au moins MIN_LARGEST bouteilles de la plus grande taille activée
// (si le volume le permet) — reproduit exactement l'ancien comportement 75cl/33cl
// quand seules ces deux tailles sont activées.
function calcOptimalBottles(volL, minLargest = 7) {
  const enabled = BOTTLE_SIZES.filter(isSizeEnabled);
  const zero = { wasteCl: 0 }; enabled.forEach(s => zero[s] = 0);
  const V = Math.round(volL * 100); // centilitres entiers
  if (!enabled.length || V <= 0) { zero.wasteCl = Math.max(0, V); return zero; }

  const sizesDesc = [...enabled].sort((a, b) => BOTTLE_SIZE_LITERS[b] - BOTTLE_SIZE_LITERS[a]);
  const largest    = sizesDesc[0];
  const largestCl  = Math.round(BOTTLE_SIZE_LITERS[largest] * 100);
  const nLargestMax = Math.floor(V / largestCl);
  const start = Math.min(minLargest, nLargestMax);
  let best = null;
  for (let n = start; n <= nLargestMax; n++) {
    let rem = V - n * largestCl;
    const counts = { [largest]: n };
    sizesDesc.slice(1).forEach(size => {
      const cl = Math.round(BOTTLE_SIZE_LITERS[size] * 100);
      counts[size] = Math.floor(rem / cl);
      rem -= counts[size] * cl;
    });
    if (best === null || rem < best.wasteCl) best = { ...counts, wasteCl: rem };
  }
  // Fallback si même start=0 n'a pas tourné (V < largestCl)
  if (!best) {
    const smallest = sizesDesc[sizesDesc.length - 1];
    const cl = Math.round(BOTTLE_SIZE_LITERS[smallest] * 100);
    const n  = Math.floor(V / cl);
    best = { [smallest]: n, wasteCl: V - n * cl };
  }
  enabled.forEach(size => { if (!(size in best)) best[size] = 0; });
  return best;
}

function _abTotalCl() {
  return BOTTLE_SIZES.reduce((s, size) => s + (parseInt(document.getElementById(`ab-${size.replace('cl','')}`)?.value) || 0) * Math.round(BOTTLE_SIZE_LITERS[size] * 100), 0);
}
...
function updateAbBottleHint() {
  ...
  const accounted  = _abTotalCl() / 100; // litres
...
function _abApplyOptimal(volBottled) {
  const opt = calcOptimalBottles(volBottled, 7);
  BOTTLE_SIZES.forEach(size => {
    const el = document.getElementById(`ab-${size.replace('cl','')}`);
    if (el) el.value = opt[size] || 0;
  });
}

function _abUpdateWraps() {
  BOTTLE_SIZES.forEach(size => {
    const wrap = document.getElementById(`ab-${size.replace('cl','')}-wrap`);
    if (wrap) wrap.style.display = isSizeEnabled(size) ? '' : 'none';
  });
}
...
function abKegChanged() {
  const volBrewed = parseFloat(document.getElementById('ab-brew-vol')?.value) || 0;
  const keg       = parseFloat(document.getElementById('ab-keg').value) || 0;
  _abApplyOptimal(Math.max(0, volBrewed - keg));
  updateAbBottleHint();
}
...
// Édition manuelle d'une taille : en mode auto, redistribue le reste dans la
// dernière taille activée (même logique que le calculateur de l'onglet Outils).
function abSizeChanged(changedSize) {
  const volBrewed  = parseFloat(document.getElementById('ab-brew-vol')?.value) || 0;
  const keg        = parseFloat(document.getElementById('ab-keg').value) || 0;
  if (!_abManualMode) {
    const volBottled = Math.max(0, volBrewed - keg);
    const enabled = BOTTLE_SIZES.filter(isSizeEnabled);
    const changedQty = Math.max(0, parseInt(document.getElementById(`ab-${changedSize.replace('cl','')}`).value) || 0);
    let rem = Math.round(volBottled * 100) - changedQty * Math.round(BOTTLE_SIZE_LITERS[changedSize] * 100);
    const others = enabled.filter(s => s !== changedSize).sort((a, b) => BOTTLE_SIZE_LITERS[b] - BOTTLE_SIZE_LITERS[a]);
    others.forEach((size, i) => {
      const cl = Math.round(BOTTLE_SIZE_LITERS[size] * 100);
      const el = document.getElementById(`ab-${size.replace('cl','')}`);
      if (i === others.length - 1) {
        el.value = rem >= cl ? Math.floor(rem / cl) : 0;
      } else {
        const n = Math.max(0, Math.floor(rem / cl));
        el.value = n;
        rem -= n * cl;
      }
    });
...
function openAfterBrewModal(id) {
  ...
  _abApplyOptimal(b.volume_brewed || 0);
...
async function createBeerFromBrew() {
  ...
  const payload = {
    ...
    ...Object.fromEntries(BOTTLE_SIZES.map(size => [`stock_${size}`, parseInt(document.getElementById(`ab-${size.replace('cl','')}`).value) || 0])),
...
async function openFermLog(brewId, btn) {
  ...
  _bflPickState = { brewId, offset: 0, limit: 100, total: 0, coches: new Set() };
  ['bfl-pick-from', 'bfl-pick-to'].forEach(id => { const e = document.getElementById(id); if (e) e.value = ''; });
  await _bflPickLoad(0);
}
...
async function _bflRefreshSources(brewId) {
  ...
  try {
    ...
    // Le sélecteur d'origine ne propose que ce que le brassin contient.
    const sel = document.getElementById('bfl-pick-source');
    if (sel) {
      sel.innerHTML = `<option value="">${t('brew.ferm_pick_any')}</option>` +
        rows.map(r => `<option value="${esc(r.source)}">${libelle[r.source] || esc(r.source)}</option>`).join('');
    }
...
// ── SÉLECTION DE RELEVÉS POINT PAR POINT ─────────────────────────────────────
// Supprimer une origine entière ne convient pas toujours : une sonde peut
// n'avoir faussé que la fin d'une courbe. On filtre, on coche, on supprime.

let _bflPickState = { brewId: null, offset: 0, limit: 100, total: 0, coches: new Set() };

function _bflPickFiltres() {
  const v = id => (document.getElementById(id)?.value || '').trim();
  return { source: v('bfl-pick-source'), from: v('bfl-pick-from'), to: v('bfl-pick-to') };
}

async function _bflPickLoad(offset) {
  const el = document.getElementById('bfl-pick');
  if (!el || !_bflPickState.brewId) return;
  _bflPickState.offset = offset;
  if (offset === 0) _bflPickState.coches.clear();
  const f = _bflPickFiltres();
  const q = new URLSearchParams({ limit: _bflPickState.limit, offset });
  if (f.source) q.set('source', f.source);
  if (f.from) q.set('from', f.from);
  if (f.to) q.set('to', f.to);
  try {
    const res = await api('GET', `/api/brews/${_bflPickState.brewId}/fermentation/readings?${q}`);
    _bflPickState.total = res.total;
    _bflPickRender(res.rows);
  } catch (e) {
    console.warn('[BrewHome] relevés :', e);
    el.innerHTML = `<div style="color:var(--danger);font-size:.85rem;padding:8px 0">${t('brew.ferm_sources_err')}</div>`;
  }
}

function _bflPickRender(rows) {
  const el = document.getElementById('bfl-pick');
  const st = _bflPickState;
  if (!rows.length) {
    el.innerHTML = `<div style="color:var(--muted);font-size:.85rem;padding:8px 0">${t('brew.ferm_pick_empty')}</div>`;
    return;
  }
  const fin = Math.min(st.offset + st.limit, st.total);
  const dt = d => d ? d.slice(8,10) + '/' + d.slice(5,7) + ' ' + d.slice(11,16) : '';
  el.innerHTML = `
    <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin:8px 0">
      <span style="font-size:.78rem;color:var(--muted)">${st.offset + 1}–${fin} / ${st.total}</span>
      <button class="btn btn-ghost btn-sm" ${st.offset === 0 ? 'disabled' : ''}
              onclick="_bflPickLoad(${Math.max(0, st.offset - st.limit)})">←</button>
      <button class="btn btn-ghost btn-sm" ${fin >= st.total ? 'disabled' : ''}
              onclick="_bflPickLoad(${st.offset + st.limit})">→</button>
      <button class="btn btn-ghost btn-sm" onclick="_bflPickToggleAll()">${t('brew.ferm_pick_all')}</button>
      <span style="flex:1"></span>
      <button class="btn btn-danger btn-sm" id="bfl-pick-del" onclick="_bflPickDelete()" disabled>
        <i class="fas fa-trash"></i> <span id="bfl-pick-n">0</span>
      </button>
    </div>
    <table style="width:100%;border-collapse:collapse;font-size:.82rem">${rows.map(r => `
      <tr style="border-bottom:1px solid var(--border)">
        <td style="padding:4px 2px;width:26px"><input type="checkbox" data-rid="${r.id}"
            ${st.coches.has(r.id) ? 'checked' : ''} onchange="_bflPickCheck(${r.id}, this.checked)"></td>
        <td style="padding:4px 2px;white-space:nowrap">${dt(r.recorded_at)}</td>
        <td style="padding:4px 2px;color:var(--muted)">${r.gravity != null ? (+r.gravity).toFixed(3) : '–'}</td>
        <td style="padding:4px 2px;color:var(--muted)">${r.temperature != null ? (+r.temperature).toFixed(1) + '°' : '–'}</td>
        <td style="padding:4px 2px;color:var(--muted);font-size:.72rem">${esc(r.source)}</td>
      </tr>`).join('')}</table>
    <div style="margin-top:8px">
      <button class="btn btn-ghost btn-sm" onclick="_bflPickDeleteMatching()">
        ${t('brew.ferm_pick_del_all').replace('${n}', st.total)}
      </button>
    </div>`;
  _bflPickSync();
}

function _bflPickCheck(id, coche) {
  if (coche) _bflPickState.coches.add(id); else _bflPickState.coches.delete(id);
  _bflPickSync();
}

function _bflPickToggleAll() {
  const cases = document.querySelectorAll('#bfl-pick input[type=checkbox]');
  const tout = [...cases].every(c => c.checked);
  cases.forEach(c => { c.checked = !tout; _bflPickCheck(+c.dataset.rid, !tout); });
}

function _bflPickSync() {
  const n = _bflPickState.coches.size;
  const btn = document.getElementById('bfl-pick-del');
  const lbl = document.getElementById('bfl-pick-n');
  if (btn) btn.disabled = n === 0;
  if (lbl) lbl.textContent = n;
}

async function _bflPickDelete() {
  const ids = [..._bflPickState.coches];
  if (!ids.length) return;
  if (!await confirmModal(t('brew.ferm_pick_confirm').replace('${n}', ids.length), { danger: true })) return;
  await _bflPickApply({ ids });
}

async function _bflPickDeleteMatching() {
  const st = _bflPickState;
  if (!st.total) return;
  const f = _bflPickFiltres();
  // Sans aucun filtre, « tout ce qui correspond » est l'historique entier :
  // on le dit, plutôt que de laisser croire à une suppression ciblée.
  const msg = (f.source || f.from || f.to)
    ? t('brew.ferm_pick_confirm_match').replace('${n}', st.total)
    : t('brew.ferm_pick_confirm_all').replace('${n}', st.total);
  if (!await confirmModal(msg, { danger: true })) return;
  const corps = { all: true };
  if (f.source) corps.source = f.source;
  if (f.from) corps.from = f.from;
  if (f.to) corps.to = f.to;
  await _bflPickApply(corps);
}

async function _bflPickApply(corps) {
  const brewId = _bflPickState.brewId;
  try {
    const res = await api('DELETE', `/api/brews/${brewId}/fermentation`, corps);
    toast(t('brew.ferm_src_deleted').replace('${n}', res.deleted), 'success');
    _bflPickState.coches.clear();
    await _bflRefresh(brewId);
    await _bflRefreshSources(brewId);
    await _bflPickLoad(0);
    const idx = S.brews.findIndex(b => b.id === brewId);
