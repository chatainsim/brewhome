...
/* En-tête « bière » des pages de recette.
   Le QR code des étiquettes mène directement à la recette : sans ce bloc, on
      ...
      <div style="margin-top:14px">
        ...
        ${byPhase[p].map(item => {
          ...
          return `<div style="display:flex;align-items:flex-start;gap:8px;padding:4px 0;border-bottom:1px solid #eee">
            ...
            <span style="font-size:10pt;${checked ? 'text-decoration:line-through;color:#888' : ''}">${esc(item.text)}</span>
          </div>`;
        }).join('')}
      </div>`).join('');
...
async function checkStaticUpdates(force = false) {
  ...
  // force : court-circuite le cache serveur (6 h) ET le cache localStorage qui
  // pilote la pastille de notification, sinon le bouton ne peut rien changer.
  if (force) { try { localStorage.removeItem('_bh_version_check'); } catch {} }
  try {
    const [data, appVer] = await Promise.all([
      api('GET', '/api/static/check-updates'),
      api('GET', '/api/version/check' + (force ? '?force=1' : '')).catch(() => null),
    ]);
