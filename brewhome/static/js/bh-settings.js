...
function exportSettings() {
  ...
  if (out.ai)       { delete out.ai.apiKey; delete out.ai.geminiApiKey; delete out.ai.openaiApiKey; }
...
async function importSettings(input) {
  ...
  try {
    ...
    const openaiBackup  = appSettings.ai?.openaiApiKey;
    ...
    if (openaiBackup && appSettings.ai) appSettings.ai.openaiApiKey = openaiBackup;
      ...
      html += `
        <div style="display:flex;align-items:center;gap:8px;background:var(--card);border:1px solid var(--border);border-radius:6px;padding:5px 10px">
          ...
          <button class="btn btn-sm btn-ghost" style="padding:2px 8px;white-space:nowrap"
            onclick="wcAddToRecipe('${mineral}',${grams.toFixed(2)},'g','${otherType}')">
    ...
    html += `
      <div style="margin-bottom:14px">
        ...
        <div style="display:flex;align-items:center;gap:8px;background:var(--card);border:1px dashed var(--border);border-radius:6px;padding:5px 10px">
          ...
          <button class="btn btn-sm btn-ghost" style="padding:2px 8px;white-space:nowrap"
            onclick="wcAddToRecipe('Chlorure de calcium',${altCaCl2g.toFixed(2)},'g','${otherType}')">
            <i class="fas fa-plus"></i> ${t('rec.wc_add_recipe')}
          </button>
        </div>
      </div>`;
    ...
    html += `
      <div style="margin-bottom:14px">
        ...
        <div style="display:flex;align-items:center;gap:8px;background:var(--card);border:1px dashed var(--border);border-radius:6px;padding:5px 10px">
          ...
          <button class="btn btn-sm btn-ghost" style="padding:2px 8px;white-space:nowrap"
            onclick="wcAddToRecipe('Carbonate de calcium',${altCaCO3g.toFixed(2)},'g','${otherType}')">
            <i class="fas fa-plus"></i> ${t('rec.wc_add_recipe')}
          </button>
        </div>
      </div>`;
    ...
    html += `
      <div style="margin-bottom:14px">
        ...
        <div style="display:flex;align-items:center;gap:8px;background:var(--card);border:1px dashed var(--border);border-radius:6px;padding:5px 10px">
          ...
          <button class="btn btn-sm btn-ghost" style="padding:2px 8px;white-space:nowrap"
            onclick="wcAddToRecipe('Bicarbonate de sodium',${altNaHCO3g.toFixed(2)},'g','${otherType}')">
            <i class="fas fa-plus"></i> ${t('rec.wc_add_recipe')}
          </button>
        </div>
      </div>`;
    ...
    activeAcids.forEach(([acid, mL]) => {
      html += `
        <div style="display:flex;align-items:center;gap:8px;background:var(--card);border:1px solid var(--border);border-radius:6px;padding:5px 10px">
          ...
          <button class="btn btn-sm btn-ghost" style="padding:2px 8px;white-space:nowrap"
            onclick="wcAddToRecipe('${acid}',${mL.toFixed(1)},'mL','${otherType}')">
            <i class="fas fa-plus"></i> ${t('rec.wc_add_recipe')}
          </button>
        </div>`;
    });
...
function wcAddToRecipe(name, qty, unit, otherType = 'empatage') {
  ...
  const existing = recIngredients.find(i => i.name === name && i.category === 'autre' && i.other_type === otherType);
  if (existing) {
