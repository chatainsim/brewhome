...
```
brewhome/
├── app.py                  Application Flask (backend, scheduler, compilation JS)
├── db.py                   Initialisation et migrations SQLite
├── helpers.py              Utilitaires partagés (validation, api_error…)
...
├── blueprints/             Routes API Flask (un fichier par ressource)
│   ├── recipes.py          Recettes et historique
│   ├── inventory.py        Inventaire
│   ├── beers.py            Cave à bières
│   ├── brews.py            Brassins
│   ├── spindles.py         Densimètres
│   ├── temperature.py      Sondes de température
│   ├── catalog.py          Catalogue d'ingrédients
│   ├── integrations.py     GitHub, Telegram, IA
│   └── ...
├── templates/
│   ├── index.html
│   └── parts/              Fragments HTML de l'interface
│       ├── scripts/        Sources JS (compilées vers static/js/ au démarrage)
│       └── styles.html     CSS global
├── static/
│   ├── js/                 JS compilés (bh-*.js, régénérés automatiquement)
│   └── ...                 CSS, fonts, icônes
